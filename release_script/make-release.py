...
def build_app(sketch_dir, flavor):
    ...
    subprocess.check_call(
        [
            "processing-java",
            "--sketch=" + sketch_dir,
            "--output=" + os.path.join(os.getcwd(), flavor),
            "--export",
        ]
    )
        ...
        try:
            subprocess.check_call(
                [
                    ...
                    "-f",
                    "-v",
                    "-s" "Developer ID Application: OpenBCI, Inc. (3P82WRGLM8)",
                    app_dir,
                ]
            )
        ...
        try:
            subprocess.check_call(
                [
                    "dmgbuild",
                    "-s",
                    "release_script/mac_only/dmgbuild_settings.py",
                    "-D",
                    "app=" + app_dir,
                    "OpenBCI_GUI",
                    dmg_dir,
...
def main():
    ...
    parser.add_argument(
        "--no-prompts",
        action="store_true",
        help="whether to prompt the user for anything",
        required=False,
    )
    parser.add_argument(
        "--pfx-path",
        type=str,
        help="path to the pfx file for windows signing",
        required=False,
        default="",
        nargs="?",
    )
    parser.add_argument(
        "--pfx-password",
        type=str,
        help="password for the pfx file for windows signing",
        required=False,
        default="",
        nargs="?",
    )
