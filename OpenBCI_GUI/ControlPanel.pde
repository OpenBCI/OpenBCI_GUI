...
class ControlPanel {
    ...
    public void CPmouseReleased() {
        ...
        if (autoSessionName.isMouseHere() && autoSessionName.wasPressed) {
            ...
            cp5.get(Textfield.class, _textField).setText(DirectoryManager.getDateString());
        }
...
public void initButtonPressed(){
    if (initSystemButton.but_txt == "START SESSION") {
        ...
        cp5.get(Textfield.class, "fileNameCyton").setText(DirectoryManager.getDateString()); //creates new data file name so that you don't accidentally overwrite the old one
        cp5.get(Textfield.class, "fileNameGanglion").setText(DirectoryManager.getDateString()); //creates new data file name so that you don't accidentally overwrite the old one
...
class SessionDataBox {
    ...
    SessionDataBox (int _x, int _y, int _w, int _h, int _padding, int _dataSource) {
        ...
        cp5.addTextfield(textfieldName)
            ...
            .setColorActive(isSelected_color)  // border color when selected
            .setColorCursor(color(26, 26, 26))
            .setText(DirectoryManager.getDateString())
            .align(5, 10, 20, 40)
            .onDoublePress(cb)
            .setAutoClear(true);
