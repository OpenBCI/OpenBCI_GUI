...
Button[] autoSessionName = new Button[2]; //0 - Cyton , 1 - Ganglion
...
class ControlPanel {
        ...
        if (systemMode != 10) {
            ...
            //active buttons during DATASOURCE_CYTON
                ...
                if (!hub.isHubRunning()) {
                    ...
                    if (autoSessionName[0].isMouseHere()) {
                        autoSessionName[0].setIsActive(true);
                        autoSessionName[0].wasPressed = true;
                    }
                ...
                if (!hub.isHubRunning()) {
                    ...
                    if (autoSessionName[1].isMouseHere()) {
                        autoSessionName[1].setIsActive(true);
                        autoSessionName[1].wasPressed = true;
                    }
    ...
    public void CPmouseReleased() {
        ...
        for (int i = 0; i < autoSessionName.length; i++) {
            autoSessionName[i].setIsActive(false);
            autoSessionName[i].wasPressed = false;
...
class SessionDataBox {
    ...
    SessionDataBox (int _x, int _y, int _w, int _h, int _padding, int _dataSource) {
        ...
        autoSessionName[i].setHelpText("Autogenerate a session name based on the date and time.");
        outputODF[i] = new Button (x + padding, y + padding*2 + 18 + 58, (w-padding*3)/2, 24, "OpenBCI", fontInfo.buttonLabel_size);
        outputODF[i].setHelpText("Set GUI data output to OpenBCI Data Format (.txt). A new file will be made in the session folder when the data stream is paused or max file size is reached.");
