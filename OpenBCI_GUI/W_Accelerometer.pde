...
class W_Accelerometer extends Widget {
    ...
    W_Accelerometer(PApplet _parent) {
        ...
        accelerometerBar.adjustTimeAxis(TimeSeriesXLim.values()[settings.tsHorizScaleSave].getValue()); //sync horiz axis to Time Series by default
...
void accelDuration(int n) {
    ...
    if (n == 0) {
        w_accelerometer.accelerometerBar.adjustTimeAxis(TimeSeriesXLim.values()[settings.tsHorizScaleSave].getValue());
    } else {
