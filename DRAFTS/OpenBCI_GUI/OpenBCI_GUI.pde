...
Channel_Figure_Component channelFigureComponents;
EEG_Processing_User eegProcessingUser;
...
void setup() {
  ...
  //if (frame != null) tResizable(true);  //make window resizable
  //attach exit handler
  //prepareExitHandler();
...
void initSystem() {
  ...
  // containers and set up CFC
  color DEFAULT_BDCOLOR = color(135, 206, 135, 50);
  color DEFAULT_MGCOLOR = color(255, 255, 255, 0);
  Container primary = new Container(0, 0, width, height, 0, DEFAULT_BDCOLOR, DEFAULT_MGCOLOR, false);
  //Container CFC_container = new Container(10, 10, 300, 300, 0, DEFAULT_BDCOLOR, DEFAULT_MGCOLOR, false);
  Container CFC_container = new Container(0.6 * width, 0.07 * height, 0.4 * width, 0.45 * height, 0, DEFAULT_BDCOLOR, DEFAULT_MGCOLOR, false);
  //Container CFC_container = new Container(primary, "RIGHT");
  channelFigureComponents = new Channel_Figure_Component(nchan, openBCI.get_fs_Hz(), CFC_container);
  
  
  eegProcessingUser = new EEG_Processing_User();
  eegProcessingUser.setup();
...
void systemUpdate() { // for updating data values and variables
  ...
  if (systemMode == 10) {
    ...
    //alternative component listener function (line 191 - 202 frame.addComponentListener()) for processing 3
    if (widthOfLastScreen != width || heightOfLastScreen != height) {
      println("OpenBCI_GUI: RESIZED");
      screenHasBeenResized = true;
      timeOfLastScreenResize = millis();
      widthOfLastScreen = width;
      heightOfLastScreen = height;
...
void systemDraw() { //for drawing to the screen
  ...
  if (systemMode == 10) {
    ...
    channelFigureComponents.draw();
    eegProcessingUser.draw();
  } else { //systemMode != 10
  ...
  if (drawPresentation) {
    myPresentation.draw();
    channelFigureComponents.drawTriggerFeedback();
  }
...
void processNewData() {
  ...
  channelFigureComponents.process(yLittleBuff_uV, dataBuffY_uV, dataBuffY_filtY_uV, fftBuff);
  eegProcessingUser.process();
