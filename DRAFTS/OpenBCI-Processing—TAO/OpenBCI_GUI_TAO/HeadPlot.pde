...
class HeadPlot {
  ...
  private int n_wide_full, n_tall_full;

// ------------------------------ for dragging purpose, added by TaoLin--------------------------------------------------------
  private boolean dragging = false; // Is the object being dragged?
  private float dragOffsetX = 0; // Mouseclick offset
  private float dragOffsetY = 0; // Mouseclick offset
  private int currentlyDragging; //the circle being dragged
// ------------------------------------------------------------------------------------------------------------
  ...
  public void setWindowDimensions(int win_width, int win_height){
    ...
    if (false) {
      ...
      n_wide_full = int(total_width); n_tall_full = int(total_height);
  ...
  private void setElectrodeLocations(int n_elec,float elec_relDiam) {
    ...
    //--------------------------------create the default 8 chan and 16 chan arrays here added by TaoLin----------------------
    //replace below elec_relXY.getFloat(i,0) and elec_relXY.getFloat(i,1) with the default x and y
...
//  }
  ...
  public void update() {
    ...
    if (false) {
      ...
      if (dragging) computePixelWeightingFactors_multiScale(n_wide_full,n_tall_full);
...
// ------------------------------ for dragging purpose, added by TaoLin --------------------------------------------------------
  ...
  void clicked(float mx, float my) {
    for (int i = 0; i < electrode_xy.length; i++) {
      ...
      if (isInsideCircle(electrode_xy[i][0], electrode_xy[i][1], (float)elec_diam, (float)elec_diam, mx, my)) {
        dragging = true;
        // If so, keep track of relative location of click to corner of rectangle
        dragOffsetX = electrode_xy[i][0]-mx;
        dragOffsetY = electrode_xy[i][1]-my;
        currentlyDragging = i;
        break;
      }
    }
  ...
  void drag(float mx, float my) {
    if (!isPixelInsideHead((int)mx, (int)my)) return;
    if (dragging) {
      electrode_xy[currentlyDragging][0] = mx + dragOffsetX;
      electrode_xy[currentlyDragging][1] = my + dragOffsetY;
    }
  }
